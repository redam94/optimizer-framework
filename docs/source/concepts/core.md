...
Models in Atlas are the predictive engines that estimate outcomes (revenue, awareness, conversions, etc.) based on budget allocations. The framework is designed to be completely model-agnostic, supporting any type of predictive model.
...
Optimization in Atlas finds the best budget allocation given your model predictions, business constraints, and objectives. The framework supports multiple optimization approaches and algorithms.
...
Constraints in Atlas define the business rules and limitations that any optimal solution must satisfy. They ensure that optimization results are feasible and aligned with business requirements.
...
Data management in Atlas handles the complex, multi-dimensional nature of budget allocation problems. The framework uses Xarray as its foundation for powerful and flexible data structures.
...
This documentation provides a comprehensive foundation for understanding and using Atlas's core concepts. Each concept is designed to be flexible and extensible while providing powerful out-of-the-box capabilities.